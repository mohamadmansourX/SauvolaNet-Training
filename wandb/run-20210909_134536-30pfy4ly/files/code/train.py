...
class ArgsParser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("-a", "--args", nargs='+', help="configuration arguments")
...
def prepare_training(config=None,model_root='expt') :
    ...
    if 'WandbCallback' in callbacks_list:
        ...
        wandb.init(project='Binarization', name=config['Global']['model_name'])
        wandb.config.update(config) 
        wb=WandbCallback()
        callbacks.append(wb)
    lr = ReduceLROnPlateau(factor=.5, min_lr=1e-7, patience=config['ReduceLROnPlateau']['patience'])
    return [ckpt, tb, es,lr], model_dir
...
def train_model():
    ...
    if os.path.isfile(config['Global']['pretrained_model']):
        model=create_multiscale_sauvola(config=config)
    else:
        model=load_pretrained_model(config=config)
    train_ds = collect_binarization_by_dataset(args.dataset)
    dg = DataGenerator(train_ds["TRAIN"], batch_size=args.batch_size, output_shape=None, nb_batches_per_epoch=len(train_ds["TRAIN"]) // args.batch_size)
    model.fit(dg,callbacks=callbacks,steps_per_epoch=len(train_ds["TRAIN"]) // args.batch_size,epochs=args.epochs)
